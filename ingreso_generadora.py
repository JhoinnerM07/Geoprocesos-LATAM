...
class EntradaDialog(QDialog):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Parámetros del Script Ingreso Generadora")
...
try:
    ...
    esquema_mavvial, esquema_placa, capa_placa, capa_mavvial = dlg.get_values()

    if not all([esquema_mavvial, esquema_placa, capa_placa, capa_mavvial]):
        raise Exception("⚠️ Todos los campos son obligatorios.")

    # ========= CARGA DEL SCRIPT =========
    mostrar_mensaje("Descargando", "📥 Descargando el script desde GitHub...")
    url_sql = "https://raw.githubusercontent.com/JhoinnerM07/Geoprocesos-LATAM/refs/heads/main/ingreso_generadora.sql"
